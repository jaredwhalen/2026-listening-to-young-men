...
/** Normalize `questions-full.csv` typology column header → quadrantKey. */
const TYPOLOGY_HEADER_TO_QUADRANT = {
	'relational/trusting': 'Trusting|Relational',
	'relational/distrusting': 'Distrusting|Relational',
	'self-driven/trusting': 'Trusting|Self-driven',
	'self-driven/distrusting': 'Distrusting|Self-driven'
};

/**
 * @param {string} columnHeader e.g. "Relational/trusting"
 * @returns {string | null} quadrantKey for {@link colorForTypologyQuadrant}
 */
export function typologyColumnToQuadrantKey(columnHeader) {
	const k = String(columnHeader ?? '')
		.toLowerCase()
		.replace(/\s+/g, '')
		.replace(/_/g, '/');
	return TYPOLOGY_HEADER_TO_QUADRANT[k] ?? null;
}
 ...
 * @returns {string}
 */
export function colorForTypologyCsvColumn(columnHeader) {
	const qk = typologyColumnToQuadrantKey(columnHeader);
	return qk ? colorForTypologyQuadrant(qk) : 'var(--color-primary)';
}
